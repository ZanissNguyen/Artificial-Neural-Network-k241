...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X) 
{
    ...
    if (m_bUse_Bias)
    {
        Y = xt::linalg::dot(X, xt::transpose(m_aWeights)) + m_aBias;
    }
    else Y = xt::linalg::dot(X, xt::transpose(m_aWeights));
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) 
{
    ...
    // delta W
    xt::xarray<double> delta_W = xt::zeros<double>({
        DY.shape()[0],
        DY.shape()[1],
        m_aCached_X.shape()[1]
    });

    // cout << getname()<< " Weights " << endl << m_aGrad_W << endl;
    for (int i = 0; i< DY.shape()[0]; i++)
    {
        xt::xarray<double> DY_i = xt::view(DY, i);
        xt::xarray<double> X_i = xt::view(m_aCached_X, i);
        xt::view(delta_W, i) = xt::linalg::outer(DY_i, X_i);
    }
    m_aGrad_W = xt::sum(delta_W, {0});

    //delta b
    if (m_bUse_Bias) m_aGrad_b = xt::sum(DY, {0});

    //delta X
    DX = xt::linalg::dot(DY, m_aWeights);
...
{
    cout << "register | " << getname() << " | weights " << endl;
    ptr_group->register_param("weights", &m_aWeights, &m_aGrad_W);
    int count = 1;
    if(m_bUse_Bias){
        cout << "register | " << getname() << " | bias " << endl;
