...
template<typename T>
xt::xarray<T> fixedShape_(xt::xarray<T> origin)
{
...
double_tensor MLPClassifier::predict(double_tensor X, bool make_decision)
{
    ...
    //DO the inference
    if (X.dimension()==1) X = fixedShape_(X);
    xt::xarray<double> Y = this->forward(X);
    ...
    bool make_decision)
{
    ...
    double_tensor results = xt::xarray<double>();
    ...
    {
        xt::xarray<double> X_batch = batch.getData();
        xt::xarray<double> Y_batch = this->forward(X_batch);

        if (make_decision) 
        {
            results =  (results==xt::xarray<double>())? Y_batch : xt::concatenate(xt::xtuple(results, Y_batch), 0);
        } else 
        {
            xt::xarray<double> Y_decision = xt::argmax(Y_batch, 1); 
            results =  (results==xt::xarray<double>()) ? Y_decision : xt::concatenate(xt::xtuple(results, Y_decision), 0);
        }
        
        // Update counters and print progress
        nsamples += X_batch.shape()[0];
        batch_idx++;
    }
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double>* pLoader)
{
    ...
    xt::xarray<double> metrics = meter.get_metrics();
    this->set_working_mode(old_mode);
    return metrics;
}
...
{
    xt::xarray<double> loss_grad = m_pLossLayer->backward();

    for (auto it = m_layers.last(); it!=m_layers.beforeFirst(); it--)
    {
        // cout << (*it)->getname() << endl;
