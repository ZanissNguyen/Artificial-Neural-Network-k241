...
protected:
    ...
    void copyFunc(const XArrayList<T> &list); // copy function
...
T XArrayList<T>::removeAt(int index)
{
    ...
    {
        if (i>=index) newData[i] = this->data[i+1];
        else newData[i] = this->data[i];
    }
    T toReturn = this->data[index];
...
bool XArrayList<T>::removeItem(T item, void (*removeItemData)(T))
{
    ...
    if (removeItemData != nullptr) (*removeItemData)(this->data[index]);
...
int XArrayList<T>::indexOf(T item)
{
    ...
    for (int i = 0; i<this->count; i++)
    {
        ...
        {
            if ((*this->itemEqual)(item, this->get(i))) return i;
        }
        else 
        {
            if (item==this->data[i]) return i;
        }
    }
...
template <class T>
void XArrayList<T>::checkAddIndex(int index)
{
    /**
...
void XArrayList<T>::ensureCapacity(int index)
{
    ...
    {
        T * newData = new T[this->capacity*2+1];
...
template <class T>
void XArrayList<T>::copyFunc(const XArrayList<T> &list)
{
    removeInternalData();
...
template <class T>
void XArrayList<T>::insert(int index, T e)
{
    if (full())
    {
        try
