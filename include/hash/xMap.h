...
KeyType deepCopyKey(const KeyType& key, std::true_type) 
{
...
template<typename KeyType>
KeyType deepCopyKey(const KeyType& key, std::false_type) 
{
    return key;
}

template<typename KeyType>
KeyType deepCopyKey(const KeyType& key) 
{
    return deepCopyKey(key, std::is_pointer<KeyType>{});
}
...
public:
    ...
    DLinkedList<Entry> pairs()
    {
        DLinkedList<Entry*> pairs(nullptr, this->valueEqual);
        for (int i = 0; i<capacity; i++)
        {
            ...
            if (list!=nullptr)
            {
                ...
                {
                    pairs.add(Entry((*it)->getKey(), (*it)->getValue()));
                }
            } 
        } 
...
xMap<K,V>::xMap(
                int (*ihashCode)(K&,int),
                float loadFactor,
                bool (*ivalueEqual)(V& lhs, V& rhs),
                void (*ideleteValues)(xMap<K,V>*),
                bool (*ikeyEqual)(K& lhs, K& rhs),
                void (*ideleteKeys)(xMap<K,V>* pMap) )
                {
    this->count = 0;
    this->capacity = 10;
    this->hashCode = ihashCode;
    this->loadFactor = loadFactor;
    this->valueEqual = ivalueEqual;
    this->deleteValues = ideleteValues;
    this->keyEqual = ikeyEqual;
    this->deleteKeys = ideleteKeys;
...
{    
    ...
    for(int idx=0; idx < map.capacity; idx++)
    {
        ...
        for(auto pEntry: list)
        {
            K newKey = deepCopyKey((*pEntry).key);
            V newValue = deepCopyKey((*pEntry).value);
...
V xMap<K,V>::put(K key, V value)
{
    ...
    if (list!=nullptr)
    {
        ...
        {
            K gotKey = (*it)->getKey();
            if (this->keyEqual!=nullptr && this->keyEqual(keyCopy, gotKey))
            {
                retValue = (*it)->getValue();
                (*it)->setValue(value);
                changed = true;
            }
            else if (gotKey == keyCopy)
            {
                retValue = (*it)->getValue();
                (*it)->setValue(value);
                changed = true;
...
V xMap<K,V>::remove(K key, void (*deleteKeyInMap)(K)){
    ...
    if (list!=nullptr)
    {
        ...
        {
            Entry * item = (*it);
            K gotKey = item->getKey();

            if (this->keyEqual!=nullptr && this->keyEqual(gotKey, keyCopy))
            {
                retValue = item->getValue();
                list->removeAt(idx);
                if (deleteKeyInMap!=nullptr) deleteKeyInMap(gotKey);
                count--;
                return retValue;
            }
            else if (gotKey==keyCopy)
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V))
{
    ...
    if (list!=nullptr)
    {
        ...
        for (auto it = list->begin(); it!=list->end(); it++)
        {
            Entry * item = (*it);
            K gotKey = item->getKey();
            V gotValue = item->getValue();
            if ((this->keyEqual!=nullptr && this->keyEqual(gotKey, keyCopy))
            ||  (gotKey==keyCopy))
            {
                if ((this->valueEqual!=nullptr && this->valueEqual(gotValue, valueCopy))
                ||  (gotValue==valueCopy))
                {
                    if (deleteKeyInMap!=nullptr) deleteKeyInMap(gotKey);
                    if (deleteValueInMap!=nullptr) deleteValueInMap(gotValue);
                    // delete item;
                    list->removeAt(idx);
                    count--;
                    return true;
...
bool xMap<K,V>::containsKey(K key){
    DLinkedList<K> keys = this->keys();
    // keys.println();
    // cout << keys.contains(key) << endl;
    // cout << keys.get(0) << endl;
    return keys.contains(key);
}

template<class K, class V>
bool xMap<K,V>::containsValue(V value){
    DLinkedList<V> values = this->values();
...
{
    DLinkedList<K> keys(nullptr, this->keyEqual);
    for (int i = 0; i<capacity; i++)
    {
...
DLinkedList<V> xMap<K,V>::values(){
    DLinkedList<V> values(nullptr, this->valueEqual);
    for (int i = 0; i<capacity; i++)
    {
        if (table[i].empty()) continue;
        DLinkedList<Entry*> * list = &table[i];
...
{
    // Remove user's data
    if (this->deleteKeys != nullptr) deleteKeys(this);
    if (this->deleteValues != nullptr) deleteValues(this);
    ...
    for (int idx = 0; idx < this->capacity; idx++) {
        ...
        for (auto& pEntry : list) {
            delete pEntry;
            pEntry = nullptr;  // Avoid double deletion
        }
        list.clear();
    }
    ...
    // Remove table
    delete[] table;
    table = nullptr;  // Avoid potential double deletion
    this->capacity = 0;
}
